...
def invert(ss):
    ...
    RetransmitFlag = bool(bin_int(pp[70:71]))
    Spare = 0
    dac = bin_int(pp[72:82])
    fi = bin_int(pp[82:88])
    BinaryData =bin_int(pp[88:])
    tt = ''
    for i in range(len(ss)):
        tt = tt + ',' + ss[i]

    return [tt, ss[0][0:10],MessageID ,RepeatIndicator ,UserID ,SeqNum ,DestinationID ,RetransmitFlag ,Spare ,dac ,fi ,BinaryData  ]